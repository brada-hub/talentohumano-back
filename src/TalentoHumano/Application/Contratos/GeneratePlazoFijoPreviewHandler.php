...
final class GeneratePlazoFijoPreviewHandler
{
    public function __construct(
        private readonly BuildPlazoFijoTemplateDataHandler $builder,
    ...
    public function handle(int $empleadoId, array $overrides = [], ?int $contratoId = null): array
    {
        ...
        $html = view('contratos.templates.plazo_fijo', $payload)->render();
    ...
    public function renderPdf(int $empleadoId, array $overrides = [], ?int $contratoId = null): array
    {
        ...
        $pdf = Pdf::loadView('contratos.templates.plazo_fijo', $payload);
        $pdf->setPaper('letter');

        $filename = 'Contrato_Plazo_Fijo_' . str_replace(' ', '_', $payload['trabajador']['nombre_completo'] ?? 'preview') . '.pdf';
