...
final class BuildPlazoFijoTemplateDataHandler
{
    ...
    public function handle(int $empleadoId, array $overrides = [], ?int $contratoId = null): array
    {
        ...
        $fechaFin = $this->toCarbon(data_get($overrides, 'contrato.fecha_fin') ?? $contrato['fecha_fin'] ?? null);
    ...
    private function formatDateLiteral(?Carbon $date): ?string
    {
        return $date ? $date->locale('es')->translatedFormat('d \\d\\e F \\d\\e Y') : null;
    }

    private function buildDuracionLiteral(?Carbon $inicio, ?Carbon $fin): ?string
    {
    ...
    private function buildLugarTrabajo(array $contrato): ?string
    {
    ...
    private function buildFullNameNamesFirst(array $persona): string
    {
        return trim(collect([
    ...
    private function buildNationalityLabel(array $persona, ?string $sexo): string
    {
    ...
    private function formatSalaryLiteral(float $value): string
    {
    ...
    private function formatSalaryNumeral(float $value): string
    {
        return 'Bs. ' . number_format($value, 0, '.', '.') . '.-';
    }

    private function formatHoursLiteral(int $hours): string
    {
        return $this->numberToWords($hours) . "({$hours})";
    }

    private function formatCountLiteral(int $number, string $singular, string $plural): string
    {
